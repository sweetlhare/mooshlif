...
export function NewAnalysisModal({
  ...
  const submit = async () => {
    ...
    setError(null)
    setQueued(0)
    const items: Array<{ file?: File; serverPath?: string }> = [
      ...files.map((f) => ({ file: f })),
      ...selectedDemos.map((d) => ({ serverPath: d.server_path })),
    ]
    let firstId = ''
    let ok = 0
    const fails: string[] = []
        ...
        {busy && (
          <div className={s.uploadState}>
            <span style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              <Spinner size={13} /> ставим в очередь… {queued} из {total}
            </span>
          </div>
        )}
